...
public class FetchItem {
   ...
   * argument, either as a protocol + hostname pair, protocol + IP address pair
   * or protocol+domain pair.
   */
  ...
  public static FetchItem create(Text url, CrawlDatum datum, String queueMode,
    ...
    if (FetchItemQueues.QUEUE_MODE_IP.equalsIgnoreCase(queueMode)) {
      try {
        ...
        InetAddress addr = IPUtils.getIP(datum);
        if (addr == null) {
          addr = InetAddress.getByName(u.getHost());
        }
      ...
      if (key == null) {
        LOG.warn(
            "Unknown domain for url: " + url + ", using URL string as key");

...
//    socket.connect(sockAddr, http.getTimeout());
      ...
      InetAddress ip = IPUtils.getIP(datum);
      ...
      if(ip == null){
        Http.LOG.info("Connecting by resolving host: " + url);
        // connect IP不存在，使用系统原有的DNS解析
        sockAddr = new InetSocketAddress(sockHost, sockPort);
        socket.connect(sockAddr, http.getTimeout());
      }else {
        Http.LOG.info("Connecting by cached ip: " + url);
        // connect IP存在，直接使用IP
        sockAddr = new InetSocketAddress(ip, sockPort);
        socket.connect(sockAddr, http.getTimeout());
      }

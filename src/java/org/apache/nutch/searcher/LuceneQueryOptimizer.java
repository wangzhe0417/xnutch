...
public LuceneQueryOptimizer(Configuration conf) {
  ...
  public TopDocs optimize(BooleanQuery original,
                          IndexSearcher luceneSearcher, int numHits,
...
//            && (luceneSearcher.docFreq(((TermQuery)c.getQuery()).getTerm())
//                / (float)luceneSearcher.maxDoc()) < threshold) { // beneath threshold
...
//                                     ChainedFilter.AND);
    ...
    if (sortField == null && !reverse) {
      ...
      if (this.searcherMaxHits <= 0 && timerThread == null)  {
        return luceneSearcher.search(query, filter, numHits);
      }
      ...
      try {
        luceneSearcher.search(query, filter, collector);
...
//          (results.totalHits*(luceneSearcher.maxDoc()/(float)exceeded.maxDoc));
      } else if (timeExceeded != null) {
        // Estimate total hits.
//        results.totalHits = (int)(results.totalHits * (luceneSearcher.maxDoc()/(float)timeExceeded.maxDoc));
      }
      return results;
...
//                             new Sort(new SortField(sortField, SortField.STRING, reverse)));
      return luceneSearcher.search(query, filter, numHits,
          new Sort(new SortField(sortField, SortField.Type.STRING, reverse)));
    }
  }
}

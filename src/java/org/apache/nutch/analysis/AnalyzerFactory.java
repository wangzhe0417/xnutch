...
public class AnalyzerFactory {
  ...
  public AnalyzerFactory(Configuration conf) {
    // TODO: 哪个为缺省？
    DEFAULT_ANALYZER = new DefaultAnalyzer();
    this.conf = conf;
    this.extensionPoint = PluginRepository.get(conf)
        .getExtensionPoint(NutchAnalyzer.X_POINT_ID);
    if (this.extensionPoint == null) {
      throw new RuntimeException(
          "x point " + NutchAnalyzer.X_POINT_ID + " not found.");
    }
    // System.out.println(this.extensionPoint.toString());
  }
  ...
  /**
   * Returns the appropriate {@link NutchAnalyzer analyzer} implementation given
   * a language code.
   *
   * <p>
   ...
   */
  public NutchAnalyzer getDefault() {
  ...
  public static void main(String[] args) throws IOException{
    String lang = "zh-cn";
    if(args.length < 2){
      System.out.println("Usage: AnalyzerFactory <langcode> <string>");
      return;
    }
    ...
    NutchAnalyzer analyzer = AnalyzerFactory.get(conf).get(lang);

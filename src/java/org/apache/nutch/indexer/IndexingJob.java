...
public class IndexingJob extends NutchTool implements Tool {
  ...
  public static final String solr = "indexer-solr";

  public static final String DONE_NAME = "index.done";

  public static final String lucene = "indexer-lucene";

  public static final String plugin = "plugin.includes";

  public static final String luceneDir = "index.lucene.dir";

  public static Boolean isSolr = false;

  public static Boolean isLucene = false;

  public static String luceneIndexDirStr = null;
  ...
  public void index(Path crawlDb, Path linkDb, List<Path> segments,
      boolean noCommit, boolean deleteGone, String params, boolean filter,
      boolean normalize, boolean addBinaryContent) throws IOException {
    index(crawlDb, linkDb, segments, noCommit, deleteGone, params, false,
        false, false, false);
  }
  ...
  public void index(Path crawlDb, Path linkDb, List<Path> segments,
      boolean noCommit, boolean deleteGone, String params, boolean filter,
      boolean normalize, boolean addBinaryContent, boolean base64)
      throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      if (str.contains(plugin)) {
        // System.out.println(conf.get(str));
        String type = conf.get(str);
        if (type.contains(solr))
          isSolr = true;
        if (type.contains(lucene))
          isLucene = true;
      } else if (str.contains(luceneDir)) {
        luceneIndexDirStr = conf.get(str);
      }
    ...
    IndexerMapReduce
        .initMRJob(crawlDb, linkDb, segments, job, addBinaryContent);
    ...
    RunningJob indexJob = null;
    if (isSolr) {
      IndexWriters writers = new IndexWriters(getConf());
      LOG.info(writers.describe());
      job.setJobName("index-solr");
      LOG.info("SOLR-INDEX");
      ...
      job.setJobName("index-lucene " + luceneIndexDirStr);
      if (luceneIndexDirStr == null) {
        LOG.info("No lucene dir are defined in your nutch-site.xml, please set property index.lucene.dir.");
        return;
      } else {
        Path luceneIndexDirPath = new Path(luceneIndexDirStr);

        LOG.info("LUCENE-INDEX");

        FileOutputFormat.setOutputPath(job, luceneIndexDirPath);

        indexJob = JobClient.runJob(job);
      }

    } else {
      LOG.info("Indexer:no indexer plugin found, please set indexer-solr or indexer-lucene in nutch-site.xml");
    }

 ...
 * @version $Id: NutchBean.java 998587 2010-09-19 04:47:40Z mattmann $
 */
...
//  private static final int MAX_OPTIMIZE_LOOPS = 3;
  ...
  public NutchBean(Configuration conf, Path dir) throws IOException {
    ...
    String url=conf.get("solr.server.url", "http://localhost:8983/solr/collection2");//"http://localhost:8983/solr/collection2";
    searchBean = new SolrBean(conf,  url);
...
//  }
  ...
  public Summary getSummary(HitDetails hit, Query query) throws IOException {
    return searchBean.getSummary(hit, query);
  }

  public Summary[] getSummary(HitDetails[] hits, Query query)
    throws IOException {
    return searchBean.getSummary(hits, query);
  }
  ...
  public String getHighlightTitle(HitDetails hit, Query query){
	    return searchBean.getHighlightedTitle(hit, query);
  }
  ...
  public static void main(String[] args) throws Exception {
    ...
    try {
      final Query query = new Query(args[0]);
      ...
      for (int i = 0; i < hits.getLength(); i++) {
        ParseData parseData = bean.getParseData(details[i]);

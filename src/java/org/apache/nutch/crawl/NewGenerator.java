 ...
 **/
public class NewGenerator extends Configured implements Tool {

  public static final Logger LOG = LoggerFactory.getLogger(NewGenerator.class);
  ...
  /// 对于可以抓取的链接转化成<主机，datum>的形式
  private static class PreSelector
    ...
    public void map(Text key, CrawlDatum value,
      ...
      if (filter) {
        ...
        try {
          ...
          if (LOG.isWarnEnabled()) {
            LOG.warn(
                "Couldn't filter url: " + url + " (" + e.getMessage() + ")");
          }
        }
      }
      ...
      try {
        ...
        crawlDatum.getMetaData().put(new Text("_url_"),
            new Text(key.toString()));

        output.collect(new Text(host), crawlDatum);
  ...
  /// 从HostDb/RobotsDb中查找IP和robots.txt
  private static class Finder
      implements Mapper<Text, CrawlDatum, Text, CrawlDatum>,
      Reducer<Text, CrawlDatum, Text, CrawlDatum> {
    
    private int maxUrlsPerHostToGenerate = 1000;
    ...
    public void reduce(Text host, Iterator<CrawlDatum> values,
      ...
      String rules = null;
      // 具有相同主机的待抓取链接
      LinkedList<CrawlDatum> urls = new LinkedList<CrawlDatum>();

      // 很多链接有相同主机
      while (values.hasNext()) {
        CrawlDatum datum = values.next();

        if (IPUtils.hasIP(datum) || RobotsUtils.hasRobots(datum)) { // 来自cacheDir
          try {
            ipStr = IPUtils.getIPString(datum);
            rules = RobotsUtils.getRobotsString(datum);
            if (ipStr != null) {
              ipFound = true;
            }
            if (rules != null) {
          ...
          if(urls.size() < maxUrlsPerHostToGenerate){
      ...
      if (ipFound || robotFound) {
        for (CrawlDatum entry : urls) {
          if (ipFound) {
            IPUtils.setIP(entry, ipStr);
          }
          if (robotFound) {
            RobotsUtils.setRobots(entry, rules);
          }
          Text url = (Text) entry.getMetaData().get(new Text("_url_"));
          if (url == null)
            continue;
          entry.getMetaData().remove(new Text("_url_"));
  ...
  public static class Selector
    ...
    public void configure(JobConf job) {
      ...
      if (GENERATOR_COUNT_VALUE_IP.equals(job.get(GENERATOR_COUNT_MODE)))
        countMode="ip";
    ...
    public void reduce(FloatWritable key, Iterator<SelectorEntry> values,
      ...
      while (values.hasNext()) {
        ...
        Text url = entry.url;
        CrawlDatum datum=entry.datum;
        String urlString = url.toString();
        URL u = null;

        String countModeValue = null;
        ...
        try {
          ...
          switch(countMode)
          {
        	  case "host":countModeValue=new URL(urlString).getHost(); break;
        	  case "domain":countModeValue=URLUtil.getDomainName(u);break; 
        	  case "ip":countModeValue=IPUtils.getIPString(datum);break;
          }
        } catch (Exception e) {
        ...
        countModeValue = countModeValue.toLowerCase();

        // only filter if we are counting hosts,domains or ips
        if (maxCount > 0) {
          int[] hostCount = hostCounts.get(countModeValue);
          if (hostCount == null) {
            hostCount = new int[] { 1, 0 };
            hostCounts.put(countModeValue, hostCount);
          }
          ...
          // reached the limit of allowed URLs per host / domain/ ip
          ...
          if (hostCount[1] >= maxCount) {
            if (hostCount[0] < maxNumSegments) {
              ...
              if (hostCount[1] == maxCount + 1 && LOG.isInfoEnabled()) {
                LOG.info(countMode +":"+ countModeValue + " has more than "
                    + maxCount + " URLs for all " + maxNumSegments
   ...
   */
  public static class CrawlDbUpdater extends MapReduceBase
      implements Mapper<Text, CrawlDatum, Text, CrawlDatum>,
      Reducer<Text, CrawlDatum, Text, CrawlDatum> {
    long generateTime;
  ...
  public NewGenerator(Configuration conf) {
    setConf(conf);
  }

  public Path[] generate(Path dbDir, Path cacheDir, Path segments, int numLists,
    ...
    return generate(dbDir, cacheDir, segments, numLists, topN, curTime, filter,
        normalise, false, 1);
  }
   ...
   **/
  public Path[] generate(Path dbDir, Path cacheDir, Path segments, int numLists,
      long topN, long curTime, boolean filter, boolean force)
      throws IOException {
    return generate(dbDir, cacheDir, segments, numLists, topN, curTime, filter,
        true, force, 1);
  }
  ...
  public Path[] generate(Path dbDir, Path cacheDir, Path segments, int numLists,
      long topN, long curTime, boolean filter, boolean norm, boolean force,
    ...
    LOG.info("Generator: Preselecting urls due for fetch.");
    ...
    String restrictStatus = getConf().get(GENERATOR_RESTRICT_STATUS, null);
    if(restrictStatus != null)
      LOG.info(GENERATOR_RESTRICT_STATUS + ":" + restrictStatus);
    ...
    // 将链接转换成主机
    JobConf jobPreselect = new NutchJob(getConf());
    jobPreselect.setJobName("generate: select from " + dbDir);

    jobPreselect.setLong(GENERATOR_CUR_TIME, curTime);
    // record real generation time
    long generateTime = System.currentTimeMillis();
    jobPreselect.setLong(Nutch.GENERATE_TIME_KEY, generateTime);
    jobPreselect.setBoolean(GENERATOR_FILTER, filter);
    jobPreselect.setBoolean(GENERATOR_NORMALISE, norm);

    FileInputFormat.addInputPath(jobPreselect,
        new Path(dbDir, CrawlDb.CURRENT_NAME));
    jobPreselect.setInputFormat(SequenceFileInputFormat.class);

    jobPreselect.setMapperClass(PreSelector.class);

    // LOG.info(tempDir.toString());

    FileOutputFormat.setOutputPath(jobPreselect, tempDir);
    jobPreselect.setOutputFormat(SequenceFileOutputFormat.class);
    jobPreselect.setOutputKeyClass(Text.class);
    jobPreselect.setOutputValueClass(CrawlDatum.class);

    try {
      JobClient.runJob(jobPreselect);
    } catch (IOException e) {
    ...
    LOG.info("Generator: Retrieving IP and robots.txt from cacheDir.");

    Path tempDir2 = new Path(getConf().get("mapred.temp.dir", ".")
        + "/generate-temp-" + java.util.UUID.randomUUID().toString());

    JobConf jobFinder = new NutchJob(getConf());
    jobFinder.setJobName("generate: match from " + cacheDir);

    FileInputFormat.addInputPath(jobFinder,
        new Path(cacheDir, HostDb.CURRENT_NAME));
    FileInputFormat.addInputPath(jobFinder, tempDir);
    jobFinder.setInputFormat(SequenceFileInputFormat.class);

    jobFinder.setMapperClass(Finder.class);
    jobFinder.setReducerClass(Finder.class);

    // LOG.info(tempDir2.toString());

    FileOutputFormat.setOutputPath(jobFinder, tempDir2);
    jobFinder.setOutputFormat(SequenceFileOutputFormat.class);
    jobFinder.setMapOutputKeyClass(Text.class);
    jobFinder.setMapOutputValueClass(CrawlDatum.class);
    jobFinder.setOutputKeyClass(Text.class);
    jobFinder.setOutputValueClass(CrawlDatum.class);

    try {
      JobClient.runJob(jobFinder);
    } catch (IOException e) {
      LockUtil.removeLockFile(fs, lock);
    ...
    LOG.info("Generator: Selecting best urls due for fetch.");

    fs.delete(tempDir,true);

    tempDir = new Path(getConf().get("mapred.temp.dir", ".") + "/generate-temp-"
    ...
    FileInputFormat.addInputPath(job, tempDir2);
    ...
    if (getConf().getBoolean(GENERATE_UPDATE_CRAWLDB, false)) {
      ...
      Path tempDir3 = new Path(getConf().get("mapred.temp.dir", ".")
          + "/generate-temp-" + java.util.UUID.randomUUID().toString());
      ...
      FileOutputFormat.setOutputPath(job, tempDir3);
      try {
        JobClient.runJob(job);
  ...
  public static void main(String args[]) throws Exception {
    int res = ToolRunner.run(NutchConfiguration.create(), new NewGenerator(),
        args);
    System.exit(res);
  }
  ...
  public int run(String[] args) throws Exception {
    if (args.length < 3) {
      System.out.println(
          "Usage: NewGenerator <crawldb> <cachedb> <segments_dir> [-force] [-topN N] [-numFetchers numFetchers] [-adddays numDays] [-noFilter] [-noNorm][-maxNumSegments num]");
      System.out.println(
          "\t * cachedb:\t hostdb/robotsdb");
      return -1;
    }

    Path dbDir = new Path(args[0]);
    Path cacheDir = new Path(args[1]);
    Path segmentsDir = new Path(args[2]);
    ...
    try {
      Path[] segs = generate(dbDir, cacheDir, segmentsDir, numFetchers, topN,
          curTime, filter, norm, force, maxNumSegments);
      if (segs == null)
        return -1;
    } catch (Exception e) {
